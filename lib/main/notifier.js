...
class Notifier {
  ...
  listenRendererMessage() {
    electron.ipcMain.on('notifier.notify', async (event, ...args) => {
      const option = args[0];
      // console.log(event.processId, event.frameId);
      this.notify(option);
  ...
  notify(options) {
    ...
    notifyOptions.icon = typeof notifyOptions.icon === 'undefined' ? this.options.icon : notifyOptions.icon;
    notifyOptions.duration = typeof notifyOptions.duration === 'undefined' ? this.options.duration : notifyOptions.duration;
    this.size = electron.screen.getPrimaryDisplay().size;
    let windowOptions = {
      width: this.options.width,
      height: this.options.height,
      x: this.size.width - this.options.width - this.options.marginRight,
      y: 200,
      frame: false,
      resizable: false,
      alwaysOnTop: true,
  ...
  checkAvailableHeight() {
    let availableHeight = this.size.height - (is.windows() ? this.startBarSizeEstimate : 0);
  ...
  showCurrent(notification) {
    const notifyOptions = notification.notifyOptions;
    let notificationWindow = notification.window;

    let notificationY = this.size.height;
  ...
  onNotificationClose(notification) {
    ...
    if (notification.window) {
      notification.window.removeAllListeners();
