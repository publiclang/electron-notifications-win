...
class Notifier {
  ...
  notify (title, data) {
    ...
    let windowOptions = {
      width: CFG_WIDTH,
      height: CFG_HEIGHT,
      x: size.width - CFG_WIDTH - CFG_MARGIN_RIGHT,
      y: 200,  /* this is because we set the poper position when we pop it from the queue */
      frame: false,
      resizable: false,
      alwaysOnTop: true,
      skipTaskbar: true,
      ...
      focusable: false,
      show: false //一开始不显示窗口，等加载并移动到目的位置后再显示，否则会闪烁
    };
    if (!is.macOS()) {
